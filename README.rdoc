...
To navigate an Ancestry model, use the following methods on any instance / record:
  ...
  self_or_ancestor_ids         Returns a list the path ids, starting with the root id and ending with the node's own id
  self_or_ancestors            Scopes model on path records of the record
